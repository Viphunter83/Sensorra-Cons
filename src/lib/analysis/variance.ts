...
export async function calculateVariance(tenderId: string): Promise<VarianceResult[]> {
    ...
    if (!(tender as any)?.project_id) return [];
    ...
    const { data: masterItems } = await supabase.from('boq_items').select('*').eq('master_boq_id', (masterBoq as any).id);
    ...
    for (const master of (masterItems as any[])) {
        ...
        for (const bid of (bids as any[])) {
            ...
            const match = (bidItems as any[])?.find(bi => bi.bid_id === bid.id && bi.master_item_id === master.id);
